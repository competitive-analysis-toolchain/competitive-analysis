...
for start_idx in [0,1,2]:
    ...
    current_X = vertcat(ref_path['x'][start_idx], ref_path['y'][start_idx], 0)
    ...
    x_history.append(x_sol[0])
    y_history.append(y_sol[0])
    theta_history.append(theta_sol[0])

    tracking_error[0] = sol.value(ocp.objective)

    i=0
    write_array = [x_sol[i], y_sol[i], theta_sol[i], delta_sol[i], V_sol[i]]
    
    #------------------------------#
    #  Code to add reference path  #
    #------------------------------#

    # for idx in range(len(ref_path['x'])):
    #     write_array.append(ref_path['x'][idx])
    #     write_array.append(ref_path['y'][idx])

    #--------------------------------------------#
    #  Code to add distance to reference points  #
    #--------------------------------------------#

    amount_of_ref_path_points = 3
    close_index = find_closest_point([x_sol[i], y_sol[i]], ref_path, start_idx)
    for steps_foward in range(amount_of_ref_path_points):
        x_dis = ref_path['x'][close_index+steps_foward]-x_sol[i]
        y_dis = ref_path['y'][close_index+steps_foward]-y_sol[i]
        write_array.append(x_dis)
        write_array.append(y_dis)

    with open('data_set_motion_planning_MPC.csv', "a+") as output:
    ...
    for i in range(Nsim-start_idx):
        ...
        j=0
        write_array = [x_sol[j], y_sol[j], theta_sol[j], delta_sol[j], V_sol[j]]
        #------------------------------#
        #  Code to add reference path  #
        #------------------------------#
        ...
        close_index = find_closest_point([x_sol[j], y_sol[j]], ref_path, start_idx)
        for steps_foward in range(amount_of_ref_path_points):
            if close_index+steps_foward<31:
                x_dis = ref_path['x'][close_index+steps_foward]-x_sol[j]
                y_dis = ref_path['y'][close_index+steps_foward]-y_sol[j]
            else:
                x_dis = ref_path['x'][30]-x_sol[j]
                y_dis = ref_path['y'][30]-y_sol[j]
            write_array.append(x_dis)
            write_array.append(y_dis)
    ...
    with open('MPC_x_motion_planning.csv', "a+") as output:
        writer = csv.writer(output, lineterminator='\n')
        writer.writerow(x_history)

    with open('MPC_y_motion_planning.csv', "a+") as output:
        writer = csv.writer(output, lineterminator='\n')
        writer.writerow(y_history)

    with open('MPC_theta_motion_planning.csv', "a+") as output:
        writer = csv.writer(output, lineterminator='\n')
        writer.writerow(theta_history)
...
for k in range(Nsim+1-start_idx):
