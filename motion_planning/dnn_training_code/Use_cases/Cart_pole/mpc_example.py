...
import csv
start_positions = linspace(-0.5, 0.5, num=200)
for idx, start_pos in enumerate(start_positions):
    ...
    current_X = vertcat(start_pos,0,0,0)  # initial state
    final_X   = vertcat(final_place,0,0,0)    # desired terminal state
    ...
    dpos_history    = np.zeros(Nsim+1)
    dtheta_history  = np.zeros(Nsim+1)
    ...
    # Lagrange objective
    ocp.add_objective(ocp.integral(F**2 + 100*(final_place-pos)**2))

    # Path constraints
    ocp.subject_to(-100 <= (F <= 100 ))
    ocp.subject_to(-10 <= (pos <= 2))
    ...
    dpos_history[0]   = current_X[2]
    dtheta_history[0] = current_X[3]
    # -------------------------------
    ...
    with open('data_set_mpc_example_bad_neg.csv', "a+") as output:
        writer = csv.writer(output, lineterminator='\n')
        writer.writerow([pos_history[0], theta_history[0], dpos_history[0], dtheta_history[0], Fsol[0]])
    ...
    for i in range(Nsim):
        ...
        dpos_history[i+1]   = current_X[2].full()
        dtheta_history[i+1] = current_X[3].full()
        F_history[i]       = Fsol[0]

        with open('data_set_mpc_example_bad_neg.csv', "a+") as output:
            writer = csv.writer(output, lineterminator='\n')
            writer.writerow([pos_history[i+1], theta_history[i+1], dpos_history[i+1], dtheta_history[i+1], Fsol[0]])
