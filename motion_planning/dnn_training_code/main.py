...
def parse_args():
    ...
    parser.add_argument("--amount_of_reference_points",default=0,help="reference points for motion planning",type=int)
...
if __name__ == "__main__":
    ...
    use_case = Motion_planning_Use_case()
    use_case.set_self_parameters()
    opt = parse_args()
    main(opt)
    print(opt.hypertune)
    retrain_for_cegar(use_case, opt.output_file, len(opt.input_variables), len(opt.output_variables), opt.amount_interval_points, opt.amount_of_reference_points, opt.old_dataset, opt.hyper_training_epochs, opt.hyper_training_factor, opt.NN_training_epochs, opt.output_folder, str2bool(opt.hypertune), opt.hyperparameter_file)
    #retrain_for_cegar(use_case, "retraining.csv", 3, 3, 4, 3, "mpc_dataset.csv", 5, 3, 10, "retraining_output", False, "hyperparameters")
