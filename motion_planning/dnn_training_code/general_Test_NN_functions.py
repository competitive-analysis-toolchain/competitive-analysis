...
def Evaluate_NN_with_MPC(use_case: Use_Case, saved_NN_weights: str, hyper_parameters_file: str, saved_Dataset: str, save_place_new_dataset: str, save_place_MSE: str, step_size_polish: int, clone_existing_data: bool =True, generate_extra_data: bool =True):
    ...
    for idx, perfect_path in enumerate(perfect_paths):
        ...
        Test_data_set = generate_data_for_start_position(use_case, NN, perfect_path.give_input_row(0), idx, use_case.timesteps[idx], save_place_new_dataset, use_case.end_iterations[idx], generate_extra_data, step_size_polish)
...
def generate_data_for_start_position(use_case: Use_Case, NN: tf.keras.Sequential, start_position: List[float], start_position_index, time_step: float, save_place_new_dataset: str, end_iterations: List[int], generate_extra_data: bool, step_size_polish: int) -> Data_set:
    ...
    if generate_extra_data:
        ...
        NN_Data_set.save_data_with_index(save_place_new_dataset, start_position_index)
...
def get_control_action_from_NN(use_case: Use_Case, NN: tf.keras.Sequential, input: List[float]) -> List[float]:
    ...
    if len(use_case.labels_output) == 1:
        ...
        for idx in range(len(use_case.labels_output)):
            NN_output_list.append(NN_output.numpy()[0][idx])
