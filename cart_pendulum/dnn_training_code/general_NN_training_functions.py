...
def get_and_process_data(saved_data: str, NN_labels_input: List[str], labels_output: List[str], train_and_test_frac: float = 0.8) -> List[pd.DataFrame]:
    ...
    for output_label in labels_output:
        output_array_train = train_features.pop(output_label)
        train_labels.append(output_array_train)

        output_array_test = test_features.pop(output_label)
        test_labels.append(output_array_test)
...
def plot_predict_vs_real(test_labels: pd.DataFrame, test_predictions: pd.DataFrame, idx: int, output_label: str, map_name: str, only_one_output: bool) -> List[float]:
    ...
    test_labels_output = []
    for i in range(len(test_labels[idx].values)):
        test_labels_output.append(test_labels[idx].values[i])
    ...
    if only_one_output:
        ...
        for i in range(len(test_predictions[idx])):
            test_predictions_output.append(test_predictions[idx][i][0])
