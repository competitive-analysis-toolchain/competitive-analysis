...
class Motion_planning_Use_case(Use_Case):
    def set_self_parameters(self):
        ...
        self.NN_labels_input = ['X','Y','THETA', 'refp_1_x', 'refp_1_y', 'refp_2_x', 'refp_2_y', 'refp_3_x', 'refp_3_y']
        self.labels_output = ['DELTA','V']
    ...
    def give_expert_actions(self, input):
        ...
        for idx in range(len(input[0])):
            [delta, V] = self.give_expert_action([input[0][idx], input[1][idx], input[2][idx]])

            output_delta.append(delta)
            output_V.append(V)

        return [output_delta, output_V]
