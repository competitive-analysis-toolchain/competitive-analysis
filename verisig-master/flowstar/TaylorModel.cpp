...
Json::Value TaylorModel::dump_json(const  std::vector<std::string> & varNames) const{
    Json::Value out;
    Json::Value coefficients = Json::arrayValue;

    Json::Value exponents = Json::arrayValue;

    for (auto f : expansion.monomials){

        std::string coefficient;
        Json::Value interval = Json::arrayValue;
        interval.append(f.coefficient.inf());
        interval.append(f.coefficient.sup());
       //  f.coefficient.toString(coefficient);
        coefficients.append(interval);
        Json::Value deg;
        for(auto d : f.getDegrees()){
            deg.append(d);
        }
    ...
    out["degrees"] = exponents;


    Json::Value rem = Json::arrayValue;
    rem.append(remainder.inf());
    rem.append( remainder.sup());
    out["remainder"] = rem;
...
/**
 * Dumps the TaylorModelVec into a json value from jsoncpp
 * @param stateVarNames the names of the variables of the hybrid automaton
 * @param tmVarNames the local variables of the taylormodels
 * @return a json value containing the necessary information information.
 */
Json::Value TaylorModelVec::dump_json(const std::vector<std::string> & stateVarNames, const std::vector<std::string> & tmVarNames) const{
    Json::Value out;
   // std::cout << "TEST " << tms.size() << " " << stateVarNames.size()<< std::endl;
    for(int i=0; i<tms.size(); ++i){
        out[stateVarNames[i]] = tms[i].dump_json(tmVarNames);
    }
    return out;
}
