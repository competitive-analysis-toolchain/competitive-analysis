...
public:
    ...
    void to_json(FILE *fp,  const std::vector<std::string> & modenames) const;
...
public:
	...
	int reach_hybrid(std::list<std::list<TaylorModelVec> > & flowpipes, std::list<std::list<std::vector<Interval> > > & domains,
            ...
            Json::Value &fp_meta
            , std::vector<int> &branchids, std::vector<FlowpipeInfo> &fpinf
            );
...
//	std::list<std::list<std::vector<Interval> > > domains;
  ...
  //  std::list<FP_meta> fp_meta_data = {};
    ...
    std::vector<FlowpipeInfo> flowpipeinfo;
