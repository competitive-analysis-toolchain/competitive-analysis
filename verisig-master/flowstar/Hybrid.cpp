		...
		const std::vector<PolynomialConstraint> & unsafeSet, const bool bSafetyChecking, const bool bPlot, const bool bDump) const
{
    ...
    printf("T::reach_continuous_non_polynomial_taylor fixed step sizes and orders 1");
	std::vector<Interval> step_exp_table, step_end_exp_table;
	Interval intZero;
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		...
		else if(res == 1)
		{
            ...
            printf("T::Printing fp to file with branchid: %d", dnn::curBranchId);
...
int HybridSystem::reach_continuous_non_polynomial_taylor(std::list<TaylorModelVec> & flowpipes, std::list<std::vector<Interval> > & domains, std::list<int> & flowpipes_safety,
		...
		const std::vector<PolynomialConstraint> & unsafeSet, const bool bSafetyChecking, const bool bPlot, const bool bDump) const
{
    printf("T::reach_continuous_non_polynomial_taylor fixed step sizes and orders 2");
...
int HybridSystem::reach_continuous_non_polynomial_taylor(std::list<TaylorModelVec> & flowpipes, std::list<std::vector<Interval> > & domains, std::list<int> & flowpipes_safety,
		...
		const std::vector<PolynomialConstraint> & unsafeSet, const bool bSafetyChecking, const bool bPlot, const bool bDump) const
{
    printf("T::reach_continuous_non_polynomial_taylor adaptive step sizes and fixed orders 1");
...
int HybridSystem::reach_continuous_non_polynomial_taylor(std::list<TaylorModelVec> & flowpipes, std::list<std::vector<Interval> > & domains, std::list<int> & flowpipes_safety,
		...
		const std::vector<PolynomialConstraint> & unsafeSet, const bool bSafetyChecking, const bool bPlot, const bool bDump) const
{
    printf("T::reach_continuous_non_polynomial_taylor adaptive step sizes and fixed orders 2");
	std::vector<Interval> step_exp_table, step_end_exp_table;
	Interval intZero;
...
int HybridSystem::reach_continuous_non_polynomial_taylor(std::list<TaylorModelVec> & flowpipes, std::list<std::vector<Interval> > & domains, std::list<int> & flowpipes_safety,
		...
		const std::vector<PolynomialConstraint> & unsafeSet, const bool bSafetyChecking, const bool bPlot, const bool bDump) const
{
    printf("T::reach_continuous_non_polynomial_taylor adaptive orders and fixed step sizes 1");
	std::vector<Interval> step_exp_table, step_end_exp_table;
	Interval intZero;
...
int HybridSystem::reach_continuous_non_polynomial_taylor(std::list<TaylorModelVec> & flowpipes, std::list<std::vector<Interval> > & domains, std::list<int> & flowpipes_safety,
		...
		const std::vector<PolynomialConstraint> & unsafeSet, const bool bSafetyChecking, const bool bPlot, const bool bDump) const
{
    printf("T::reach_continuous_non_polynomial_taylor adaptive orders and fixed step sizes 2");
	std::vector<Interval> step_exp_table, step_end_exp_table;
	Interval intZero;
...
int HybridSystem::reach_hybrid(std::list<std::list<TaylorModelVec> > & flowpipes, std::list<std::list<std::vector<Interval> > > & domains,
		...
		const std::vector<std::vector<PolynomialConstraint> > & unsafeSet, const bool bSafetyChecking, const bool bPlot, const bool bDump, Json::Value &fp_meta, std::vector<int> &branchids, std::vector<FlowpipeInfo> &fpinf)
{
	...
	for(; modeQueue.size() != 0;)
	{
		...
		case NONPOLY_TAYLOR:
		{
            printf("T::nonpoly taylor case\n");
			switch(orderType)
			{
			case UNIFORM:
                printf("T::uniform case\n");
				if(bAdaptiveSteps)
				{
        ...
        std::cout << "T:::" << branchids.size() << std::endl;

        branchids.push_back(dnn::curBranchId);
        ////// OUTPUTTING NEW FLOWPIPE TO JSON //////


        std::cout << "T::: flowpipes size: " << flowpipes.size() << std::endl;
        std::cout << "T::: domains size:   " << domains.size() << std::endl;
        std::cout << "T::: last flowpipe size: " << flowpipes.back().size() << std::endl;
        std::cout << "T::: last domain size: " << domains.back().size() << std::endl;


        //fp_meta["FLOWPIPES"]

        /* todump
        static unsigned long fp_id = 0;

        std::string fpname = "flowpipe" + std::to_string(fp_id++);


        fp_meta[fpname]["mode"] = modeNames[initMode];
        fp_meta[fpname]["modeID"] = initMode;
        fp_meta[fpname]["jumpsExecuted"] = jumpsExecuted;
        fp_meta[fpname]["branchID"] = dnn::curBranchId;

        if(flowpipes.size() > 0){
            // add flowpipe
            auto last_fp = flowpipes.back().back();
            fp_meta[fpname]["Flowpipe"] =  last_fp.dump_json(stateVarNames, tmVarNames);

            // add domains
            auto last_d = domains.back().back();
            Json::Value dom;
            for(Interval i : last_d){
                Json::Value interval;
                interval.append(i.inf());
                interval.append(i.sup());
                dom.append(interval);
            }
            fp_meta[fpname]["domains"] = dom;
        }



        if (result == COMPLETED_UNSAFE){
            fp_meta[fpname]["unsafe"] = true;
        } else {
            fp_meta[fpname]["unsafe"] = false;
        }
         */

        FlowpipeInfo newentry;
        newentry.branchid = dnn::curBranchId;
        newentry.jumpsexecuted = jumpsExecuted;
        newentry.modeid = initMode;
        newentry.modename = modeNames[initMode];
        newentry.safe = (result == COMPLETED_UNSAFE);
        fpinf.push_back(newentry);


/*      Removed, since we use json now
        ////// TRYING OUTPUTTING FLOWPIPES HERE ////////

        if (flowpipes.size() > 0){
            //print latest flowpipe

#ifdef PRINT_MPC_VERIF

            FILE *fl_file;
            fl_file = fopen("flowpipes.out", "a");

            fprintf(fl_file, "-\n");
            fprintf(fl_file, "mode: %s,\t", modeNames[initMode].c_str());
            fprintf(fl_file, "jumps executed: %d\t", jumpsExecuted);
            fprintf(fl_file, "branchid: %d\n", dnn::curBranchId);

            std::list<TaylorModelVec> c_fp = flowpipes.back();


            TaylorModelVec fp = c_fp.back();
            //std::list<TaylorModelVec>::iterator fpIter = c_fp.begin();

            fprintf(fl_file, "PRINTING FLOWPIPES\n");


            //for(; fpIter != c_fp.end(); ++fpIter){
            //    fpIter->dump_interval(fl_file, stateVarNames, tmVarNames);
            //}

            fp.dump_interval(fl_file, stateVarNames, tmVarNames);
...
*/
		...
		{
            std::cout << "TTTTT:: result on uncompleted_safe or completed_unsafe" << result << std::endl;

            std::cout << "TTTTT:: " << COMPLETED_UNSAFE << std::endl;
		...
		for(int i=0; i<transitions[initMode].size(); ++i)
		{
			...
			if(bPrint)
			{
             //   transitions[initMode][i].jumpID;
                printf("custom output: from: %s, to: %s, jumps execuded: %d, branch id: %d, jump id: %d \n", modeNames[initMode].c_str(), modeNames[transitions[initMode][i].targetID].c_str(), jumpsExecuted, dnn::curBranchId, transitions[initMode][i].jumpID);
...
HybridReachability::HybridReachability()
{
    ...
    std::cout << "TT:: i pushed back" << std::endl;
	traceTree = NULL;
	numOfJumps = 0;
    fp_meta_filename = "./outputs/flowpipes.json";
}
...
void HybridReachability::dump(FILE *fp) const
{
    ...
    std::list<std::list<TaylorModelVec> >::const_iterator fpit = flowpipes.begin();
    std::list<std::list<std::vector<Interval> >  >::const_iterator domit = domains.begin();
    // write all flowpipes to json
    for(uint index = 0; index < flowpipes.size(); index++){
        std::string fp_name = "flowpipe" + std::to_string(index);
        //add meta data
        jsonout[fp_name]["mode"] = flowpipeinfo[index].modename;
        jsonout[fp_name]["modeID"] = flowpipeinfo[index].modeid;
        jsonout[fp_name]["jumpsExecuted"] = flowpipeinfo[index].jumpsexecuted;
        jsonout[fp_name]["branchID"] = flowpipeinfo[index].branchid;
        jsonout[fp_name]["unsafe"] = flowpipeinfo[index].safe;

        // add flowpipe
        if (fpit->size() > 0){
            auto fplast = fpit->back();
            jsonout[fp_name]["flowpipe"] = fplast.dump_json(stateVarNames, tmVarNames);
        }

        // add domains
        auto domlast = domit->back();
        Json::Value doms;
        for(Interval i : domlast){
            Json::Value interval;
            interval.append(i.inf());
            interval.append(i.sup());
            doms.append(interval);
        }

        jsonout[fp_name]["domains"] = doms;
    ...
    // output branching factor to json.
    Json::Value transitions;
    for(auto t_list : this->system.transitions) {
        for (auto trans: t_list) {
            Json::Value new_t;
            new_t.append(trans.startID);
            new_t.append(trans.targetID);
            transitions.append(new_t);
        }
    }
    jsonout["transitions"] = transitions;
    ...
    fprintf(fp, "TraceTree JSON \n");
    traceTree->to_json(fp, modeNames);
    fprintf(fp, "JSON END\n");


    //traceTree->to_dot(fp, 0, modeNames);
    ...
    std::cout << "------- CUSTOM OUTPUT -------" << std::endl;
    std::cout << "length of flowpipes: " << flowpipes.size() << std::endl;
    std::cout << "length of each flowpipe:";

    for(auto test : flowpipes){
        std::cout << test.size() << ", ";
    }
    std::cout << std::endl;
...
//	build_constraint_template((int)(stateVarNames.size()));
	...
	return system.reach_hybrid(flowpipes, domains, flowpipes_safety, flowpipes_contracted, num_of_flowpipes, modeIDs, traceNodes, traceTree, integrationSchemes, time, maxJumps,
			...
			linear_auto, template_auto, bPrint, stateVarNames, modeNames, tmVarNames, unsafeSet, bSafetyChecking, bPlot, bDump, fp_meta, branchids, flowpipeinfo);
}
...
/* Prints trace Tree in JSON format to json file.*/
void TreeNode::to_json(FILE *fp,  const std::vector<std::string> & modenames) const{
    fprintf(fp, "{\n");
    fprintf(fp,"\"mode\": \"%s\", \n", modenames[modeID].c_str());
    fprintf(fp,"\"modeID\": %d,\n", modeID);
    fprintf(fp,"\"jumpID\":%d,\n",jumpID);
    fprintf(fp,"\"interval\": [%f, %f], \n",localTime.inf(), localTime.sup());
    fprintf(fp,"\"children\": [\n");
    if(children.size() == 0)
    {
       // printf("no more children found\n");
    }
    else
    {
        std::list<TreeNode *>::const_iterator iter = children.begin();
